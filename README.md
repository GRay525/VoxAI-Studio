...
## VoxAI Studio Desktop App

In addition to the research model and WebUI, this repository also contains **VoxAI Studio**, a modern Electron-based desktop client powered by IndexTTS2.

- **Backend (FastAPI, required)**  
  ```bash
  uv run api_server.py --host 127.0.0.1 --port 8000
  ```
...
- **Desktop App (Electron frontend)**  
  ...
  npm start          # launch VoxAI Studio
  ```

VoxAI Studio provides a bilingual (English/Chinese) interface for zero-shot TTS, emotion control, history management, and voice library operations on top of the IndexTTS2 model.
